...
RethinkDB.prototype.makeDBAndTables = function() {
  ...
  var makeUserTable = function () {
    self.user_db.tableCreate(self.options.userTable, { cache_size: 32 }).run(self.connection, function(err, result) {
      if (err)
        exitOnError(err)
      if (result.created !== 1) {
...
RethinkDB.prototype.getNodeWithParents = function(path) {
  ...
  if (path === "")
    path = "/"
  var nodeWithParentsRelation = self.db.table(uri.user.name).orderBy('path') //.filter(r.js('(function(node) { return "' + path +'".match("^"+node.path); })')).orderBy('path').map(function(node){return node('content')})
...
RethinkDB.prototype.addUser = function(name, domain, password) {
  ...
  self.isUser(name).then(function() {
    ...
    Q.ninvoke(self.db.tableCreate(name, { cache_size: 128 }), 'run', self.connection).then(function() {
