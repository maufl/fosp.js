...
ConnectionPool.prototype.getOrCreateOne = function(domain, callback) {
  ...
  try {
    ...
    newWs.connect('ws://'+domain+':'+self.server.remote_port);
    newWs.on('connect', function(con) {
      var newCon = self.server.registerConnection(con)
      newCon.sendConnect({}, {version: '0.1'}).on('succeeded', function() {
        newCon.negotiated = true
        newCon.sendAuthenticate({}, {type: 'server', domain: self.server.local_domain}).on('succeeded', function() {
          callback(null, newCon);
          newCon.updateContext('server', domain)
          newCon.authenticated = true
        }).on('failed', function() {
