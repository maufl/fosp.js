...
RethinkDB.prototype.nodeExists = function(path) {
  ...
  Q.ninvoke(self.data_table.filter({user: uri.user.name, path: uri.path}).count(), 'run', self.connection).then(function(num) {
    if (num !== 1) {
...
RethinkDB.prototype.deleteNode = function(path) {
  ...
  d.resolve(self.nodeExists(path).then(function() {
    return Q.ninvoke(self.data_table.filter({user: uri.user.name }).filter(r.row('path').match('^'+uri.path)).delete(), 'run', self.connection)
  }))
  return d.promise
}
...
RethinkDB.prototype.listChildren = function(path) {
  ...
  if (path === '/')
    path = ''
  self.data_table.filter({user: uri.user.name }).filter(r.row('path').match('^'+path+'/[^/]+$')).run(self.connection, function(err, cursor) {
    if (err) {
      d.reject(db_error(err))
      return
    }
...
RethinkDB.prototype.getNodeWithParents = function(path) {
  ...
  if (path === "")
    path = "/"
  var nodeWithParentsRelation = self.data_table.filter({ user: uri.user.name }).orderBy('path')
