...
var ServerConnection = function(ws) {
  ...
  self.on('request', function(msg) {
    L.info('Recieved request ' + msg.short())
    L.debug('Recieved request ' + msg.toString())
    P.log('Entering ServerConnection request event')
    switch(msg.request) {
      case 'CONNECT':
...
ServerConnection.prototype.sendMessage = function(msg) {
  L.info('Sending ' + Message.TYPES[msg.type] + ' ' + msg.short())
  L.debug('Sending ' + msg.toString())
  P.log('Entering ServerConnecion.sendMessage')
  return Connection.prototype.sendMessage.call(this, msg)
}
