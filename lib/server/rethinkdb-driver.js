...
RethinkDB.prototype.makeDataTableIndex = function() {
  var self = this
  return Q.ninvoke(r.db(self.options.db).table(self.options.dataTable).indexCreate('user'), 'run', self.connection).then(function() {
    return Q.ninvoke(r.db(self.options.db).table(self.options.dataTable).indexCreate('path'), 'run', self.connection)
  })
}
...
RethinkDB.prototype.nodeExists = function(path) {
  ...
  Q.ninvoke(self.data_table.getAll(uri.user.name, { index: 'user' }).filter({ path: uri.path }).count(), 'run', self.connection).then(function(num) {
    if (num !== 1) {
...
RethinkDB.prototype.createNode = function(path, content) {
  ...
  self.nodeExists(uri.fqUser() + parentPath).then(function() {
    return Q.ninvoke(self.data_table.insert({user: uri.user.name, path: uri.path, content: content}), 'run', self.connection)
  }).then(function() {
    d.resolve()
  }).catch(function(err) {
...
RethinkDB.prototype.updateNode = function(path, content) {
  ...
  var self = this, uri = new fosp.URI(path);
  return Q.ninvoke(self.data_table.filter({ user: uri.user.name, path: uri.path }).update({content: content}), 'run', self.connection)
}
...
RethinkDB.prototype.deleteNode = function(path) {
  ...
  d.resolve(self.nodeExists(path).then(function() {
    return Q.ninvoke(self.data_table.getAll(uri.user.name, { index: 'user' }).filter(r.row('path').match('^'+uri.path)).delete(), 'run', self.connection)
  }))
  return d.promise
}
...
RethinkDB.prototype.listChildren = function(path) {
  ...
  if (path === '/')
    path = ''
  self.data_table.getAll(uri.user.name, { index: 'user' }).filter(r.row('path').match('^'+path+'/[^/]+$')).run(self.connection, function(err, cursor) {
    if (err) {
      d.reject(db_error(err))
      return
    }
...
RethinkDB.prototype.getNodeWithParents = function(path) {
  ...
  if (path === "")
    path = "/"
  var nodeWithParentsRelation = self.data_table.getAll(uri.user.name, { index: 'user' }).orderBy('path')
...
RethinkDB.prototype.addUser = function(name, domain, password) {
  ...
  return self.userExists(name).then(function(exists) {
    ...
    var node = { user: name, path: '/', content: { owner: name+'@'+domain, btime: moment().toISOString(), mtime: moment().toISOString(), acl: {}, data: "Welcome home" } }
