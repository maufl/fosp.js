...
var buildModule = function(URI, Message, Request, Response, Notification, L) {
  ...
  var parseMessageBuffer = function(buffer) {
    ...
    if (i < buffer_length) {
      ...
      if (Message.BufferClass === ArrayBuffer)
        new_buffer = new Uint8Array(new_buffer)
  ...
  var messageHashToObject = function(con, msg) {
  ...
  Parser.parseMessage = function(con, raw, callback) {
    L.verbose(raw.toString())
    try {
      var msg = null;
      if ( raw instanceof Message.BufferClass ) {
        msg = parseMessageBuffer(raw)
        callback(null, messageHashToObject(con, msg))
      }
