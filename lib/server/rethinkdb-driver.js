...
RethinkDB.prototype.writeAttachment = function(resourceID, buffer, callback) {
  var self = this, file_name = (new Buffer(resourceID)).toString('base64'), full_path = path.join(self.filestorage, file_name)
  fs.writeFile(full_path, buffer, {encoding: 'binary'}, function(err) {
    if (err) {
      ...
      callback(new Error('Internal error while writing to attachment'))
    }
    callback(null)
  })
}

RethinkDB.prototype.readAttachment = function(resourceID, callback) {
  var self = this, file_name = (new Buffer(resourceID)).toString('base64'), full_path = path.join(self.filestorage, file_name)
  fs.readFile(full_path, function(err, buffer) {
    if (err) {
      ...
      callback(new Error('Internal error while reading attachment'), null)
    }
