...
DatabaseAbstractionLayer.prototype.permissionsFor = function(user, path) {
  var self = this, d = Q.defer()
  self.dbDriver.getNodeWithParents(path).then(function(nodes) {
...
DatabaseAbstractionLayer.prototype.authenticateFor = function(user, path, permissions) {
  var self = this, d = Q.defer()
  this.permissionsFor(user, path).then(function(val) {
...
DatabaseAbstractionLayer.prototype.select = function(user, path, callback) {
  var self = this
  this.dbDriver.nodeExists(path).then(function() {
    return self.permissionsFor(user, path)
  }).then(function(val) {
...
DatabaseAbstractionLayer.prototype.selectForUsers = function(users, path, callback) {
  var self = this
  this.dbDriver.nodeExists(path).then(function() {
    return self.dbDriver.getNodeWithParents(path)
  }).then(function(nodes) {
...
DatabaseAbstractionLayer.prototype.list = function(user, path, callback) {
  var self = this
  self.authenticateFor(user, path, ['children-read']).then(function(node) {
    self.dbDriver.listChildren(path).then(function(children) {
...
DatabaseAbstractionLayer.prototype.prepareNotifications = function(path, event) {
  ...
  self.dbDriver.getNodeWithParents(path).then(function(nodes) {
    ...
    if (fired)
      setTimeout(function() { self.emit(event, usersToNotify, path) }, 50)
    else
      calculated = true
  }).catch(function(err) {
...
DatabaseAbstractionLayer.prototype.create = function(user, path, content, callback) {
  ...
  self.authenticateFor(user, parentPath, neededPermissions).then(function(node) {
    self.dbDriver.nodeExists(path).then(function() {
      ...
      self.dbDriver.createNode(path, content).then(function() {
        callback(null)
...
DatabaseAbstractionLayer.prototype.update = function(user, path, content, callback) {
  ...
  self.authenticateFor(user, path, neededPermissions).then(function(node) {
    ...
    self.dbDriver.updateNode(path, node).then(function() {
      callback(null)
...
DatabaseAbstractionLayer.prototype.delete = function(user, path, callback) {
  ...
  self.authenticateFor(user, parentPath, ['children-delete']).then(function(node) {
    var notifications  = self.prepareNotifications(path, 'deleted');
    self.dbDriver.deleteNode(path).then(function() {
      callback(null)
      notifications.fire()
    }).catch(function(err) {
      callback(err)
...
DatabaseAbstractionLayer.prototype.readAttachment = function(user, path, callback) {
  var self = this
  self.authenticateFor(user, path, ['attachment-read']).then(function() {
    return self.dbDriver.readAttachment(path)
...
DatabaseAbstractionLayer.prototype.writeAttachment = function(user, path, buffer, callback) {
  var self = this
  self.authenticateFor(user, path, ['attachment-write']).then(function() {
    return self.dbDriver.writeAttachment(path, buffer)
  }).then(function() {
    callback(null)
  }).catch(function(err) {
    callback(err)
