...
var buildModule = function(events, URI, Message, Request, Parser, L) {
  ...
  var Connection = function(ws) {
    ...
    var emitMessage = function(message) {
      ...
      var msg = Parser.parseMessage(self, data, function(err, msg){
        if (err) {
          L.error('Error while parsing message: ' + err)
          L.error(err.stack)
          return
        }
    ...
    if (typeof self.ws.on === 'function') {
      ...
      self.ws.on('close', function(code, description) { self.emit('close', code, description); });
      self.ws.on('error', function(err) { self.emit('error',err); });
    }
    else {
      self.ws.onmessage = function(message) { emitMessage(message); }
      self.ws.onclose = function(ev) { self.emit('close', ev.code, ev.reason); }
    ...
    self.on('close', function(code, reason) {
      L.info('Connection ' + self.id + ' closed, code ' + code + ': ' + reason)
    })

    self.on('error', function(err) {
      L.error('Fatal! Unrecoverable error occured on connection ' + self.id)
      L.error(err.stack)
    })
  };
