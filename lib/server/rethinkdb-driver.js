...
RethinkDB.prototype.nodeExists = function(path) {
  ...
  Q.ninvoke(self.data_table.getAll(path, { index: 'uri' }).count(), 'run', self.connection).then(function(num) {
    if (num !== 1) {
...
RethinkDB.prototype.__getNode = function(path) {
  var self = this, uri = new fosp.URI(path), d = Q.defer()
  Q.ninvoke(self.data_table.getAll(path, { index: 'uri' }), 'run', self.connection).then(function(cursor) {
    cursor.toArray(function(err, result) {
      if (err)
        d.reject(err)
      if (result.length === 0) {
        var err = new Error('Node ' + path + ' not found')
        err.status_code = 404
        d.reject(err)
      }
      else {
...
RethinkDB.prototype.createNode = function(path, content) {
  ...
  self.__getNode(uri.parent().toString()).then(function(node) {
    return Q.ninvoke(self.data_table.insert({ uri: path, parent: node.id, content: content }), 'run', self.connection)
  }).then(function() {
    d.resolve()
  }).catch(function(err) {
...
RethinkDB.prototype.updateNode = function(path, content) {
  ...
  return Q.ninvoke(self.data_table.getAll(path , { index : 'uri' }).update({content: content}), 'run', self.connection)
}
...
RethinkDB.prototype.deleteNode = function(path) {
  ...
  d.resolve(self.nodeExists(path).then(function() {
    return Q.ninvoke(self.data_table.filter(r.row('uri').match('^'+path)).delete(), 'run', self.connection)
  }))
  return d.promise
}
...
RethinkDB.prototype.listChildren = function(path) {
  var self = this, d = Q.defer()
  self.__getNode(path).then(function(node) {
